...
<div id="main" class="wrap">
	...
	<div class="main">
		...
		<div class="manage">
			<div class="search">
				<form method="post" action="${pageContext.request.contextPath}/order/findOrder">
			...
			<table class="list">
				...
				<c:forEach items="${orderList.items}" var="order">
					<tr>
						...
						<td class="w1 c">
							<a href="${pageContext.request.contextPath}/order/beforeUpdate/${order.eoId}">修改</a>
							<a href="javascript:void(0);" onclick="delOrder('${order.eoId}')">删除</a></td>
					</tr>
				</c:forEach>
			</table>
			<div class="pager">
				<ul class="clearfix">
					<li>
						<a href="${pageContext.request.contextPath}/order/list/${orderList.currentPage == 1 ? orderList.currentPage : orderList.currentPage - 1}/5">上一页</a>
					</li>
					<c:forEach begin="${orderList.currentPage > 3 ? orderList.currentPage - 3 : 1}" end="${orderList.totalPages - orderList.currentPage > 3 ? orderList.currentPage + 3  : orderList.totalPages  }" var="i">
						<c:if test="${orderList.currentPage == i}">
							<li class="current"><a href="${pageContext.request.contextPath}/order/list/${i}/5">${i}</a></li>
						</c:if>
						<c:if test="${orderList.currentPage != i}">
							<li><a href="${pageContext.request.contextPath}/order/list/${i}/5">${i}</a></li>
						</c:if>
					</c:forEach>

					<li>
						<a href="${pageContext.request.contextPath}/order/list/${orderList.currentPage == orderList.totalPages ? orderList.currentPage : orderList.currentPage + 1  }/5">下一页</a>
					</li>
					<li>共 ${orderList.totalPages} 页</li>
				</ul>
			</div>
		</div>
	</div>
