...
<head>
	...
	function buy() {
		location.href="${pageContext.request.contextPath}/shopping-result.jsp";
	}
	</script>
</head>
<body>
...
<div id="main" class="wrap">
	...
	<div id="product" class="main">
		...
		<div class="infos">
			<div class="thumb">
				<img style="width: 313px; height: 296px;" src="${pageContext.request.contextPath}/images/product/${product.epFileName}" /></div>
			<div class="buy">
				<p>商城价：<span class="price">￥${product.epPrice}</span></p>
				<p>
					库　存：<c:if test="${product.epStock > 0}">有货(库存${product.epStock}件)</c:if>
							<c:if test="${product.epStock == 0}">无货(库存0件)</c:if>
				  </p>
			  <div class="button">
				  <input type="button" name="button" onclick="buy()"/>
				  <a href="${pageContext.request.contextPath}/product/addToShoppingCart/${product.epId}">放入购物车</a>
			  </div>
			</div>
			<div class="clear"></div>
		</div>
